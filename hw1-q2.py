...
class LogisticRegression(nn.Module):
    ...
    def __init__(self, n_classes, n_features, **kwargs):
        ...
        super().__init__()
        self.layer = nn.Linear(n_features, n_classes) #only linear is needed, softmax is calculated in loss function
...
class FeedforwardNetwork(nn.Module):
    ...
    def forward(self, x, **kwargs):
        ...
        for i in range(self.layers - 1):
            z = self.hidden_layer(h)
            h = self.activation(z)
            h = self.dropout(h)

        out = self.output(h) #again, no need for softmax
...
def train_batch(X, y, model, optimizer, criterion, **kwargs):
    ...
    """
    optimizer.zero_grad() # cleaning accumulated grandients

    y_p = model.forward(X) # calculating model's predicted labels

    Loss = criterion(y_p, y) #calculating loss

    Loss.backward() #computing gradients w/ respect to weights

    optimizer.step() #updating weights according to optimizer


    return Loss.item() #loss as numeric value
...
def main():
    ...
    print('Max Valid acc: %.4f' % max(valid_accs))
